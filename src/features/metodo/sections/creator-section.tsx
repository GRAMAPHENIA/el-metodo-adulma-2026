...
type CreatorSectionProps = {
	mode?: 'method' | 'training';
};

export function CreatorSection({
	mode = 'method',
}: CreatorSectionProps) {
	const showHeading = mode === 'method';
	const showAssociateCard = mode === 'training' && Boolean(associateContent);
	...
	return (
		<section className='section-spacing relative overflow-hidden bg-surface-base'>
			<Container className='relative'>
				...
				<div
					className={`grid gap-6 ${showAssociateCard ? 'mt-8 sm:grid-cols-2 lg:grid-cols-2' : 'mt-8 max-w-[38rem]'}`}
				>
